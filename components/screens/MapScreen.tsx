    ...
    <div className="h-full w-full flex flex-col md:flex-row gap-4 relative overflow-hidden">
        ...
        <div className="w-full md:w-1/2 flex flex-col items-center justify-center relative p-4 border border-pip/20 bg-pip/5 rounded">
            ...
            <div className="relative w-48 h-48 sm:w-64 sm:h-64 flex items-center justify-center">
                ...
                {/* Main Earth Icon */}
                <div className="relative z-10 bg-black rounded-full p-2 border border-pip shadow-[0_0_20px_rgba(65,255,0,0.4)]">
            ...
            <div className="mt-8 flex items-center gap-2 text-pip font-bold tracking-widest uppercase">
                ...
                <span>{t.mapScreen.positionLocked}</span>
            </div>
        </div>
        ...
        <div className="w-full md:w-1/2 flex flex-col h-full border-l-0 md:border-l-2 border-pip/30 pl-0 md:pl-4 overflow-y-auto">
            <div className="flex items-center gap-2 mb-4 border-b-2 border-pip pb-2 text-pip">
                <Navigation size={20} />
                <h2 className="text-xl font-bold uppercase tracking-wider">{t.mapScreen.localMapData}</h2>
            </div>
                ...
                {LOCATION_KEYS.map((key, index) => (
                    ...
                    >
                        ...
                        <div className="flex flex-col">
                            <span className={`font-mono uppercase ${index === 0 ? 'text-lg font-bold text-pip drop-shadow-[0_0_5px_rgba(65,255,0,0.8)]' : 'text-base text-pip/80'}`}>
                                {t.mapScreen.locations[key]}
                            </span>
                            {index === 0 && (
                                <span className="text-xs text-pip/60 uppercase tracking-widest">{t.mapScreen.currentLocation}</span>
                            )}
                        </div>
                    </div>
                ))}
